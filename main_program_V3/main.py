...
# read txt file to get important info
def readText():
    myfile = open("info.txt", "rt") 
    contents = myfile.read()    # string type     
    ...
    # read TO email
    target1 = re.search('TO=/(.+?)/', contents)     
    if target1:
        To = target1.group(1) 
        gmail_info.append(To)
    else:
        print("read TO email failed!")
        sys.exit()

    # read sender email
    target2 = re.search('gmail_sender=/(.+?)/', contents)     
    if target2:
        gmail_sender = target2.group(1) 
        gmail_info.append(gmail_sender)
    else:
        print("read gmail account failed!")
        sys.exit()

    # read sender password
    target3 = re.search('gmail_passwd=/(.+?)/', contents)     
    if target3:
        gmail_passwd = target3.group(1) 
        gmail_info.append(gmail_passwd)
    else:
        print("read gmail password failed!")
        sys.exit()
...
def sendMail():
    ...
    # read important info from text
    g_info = readText()

    # email address used to send to
    TO = g_info[0]
    ...
    # Gmail Sign In
    gmail_sender = g_info[1]
    gmail_passwd = g_info[2]
...
def cat_check_P(p_num, product_num):
    ...
    elif (p_num // product_num == 1):
        List_P.append("hand sanitizer. ")
    elif (p_num // product_num == 2):
        List_P.append("medical exam gloves. ")
    else:
        List_P.append("User's own products. ")
...
def cat_check_N(p_num, product_num):
    if (p_num // product_num == 0):
        ...
        List_N.append("User's own products. ")
...
# a function that can let user add in links they would like to detect
def usr_type_link():
    usrurl_num = input('Please type in how many links (in int) you wanna have a check. \n')
    #usrurl_num = int(usrurl_num)
    try:
        usrurl_num = int(usrurl_num)

        for i in range(usrurl_num):
            usr_link = input('Please type in product link in Amazon. \n')
            # find product ID
            target = re.search('/dp/(.+?)/ref=', usr_link)
            ...
            if target:
                ...
                print("Not a valid amazon product's link.")

    except ValueError:
        try:
            usrurl_num = float(usrurl_num)
            print("Input is a float  number. NOT accepted.")
            print("At this time, we will only give your report for best sellers.")
        except ValueError:
            print("Input is not a number. NOT accepted.")
            print("At this time, we will only give your report for best sellers.")
...
def autoDetect(recommand_url):
    ...
    # user add in links they would like to detect
    usr_type_link()
